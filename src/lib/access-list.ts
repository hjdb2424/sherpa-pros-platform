...
//
// This is a DEV-ONLY fallback. Prod always reads from the access_list
// DB table (isEmailAllowedAsync + getAccessEntryAsync fail closed in prod
// when the row isn't found). Real testers go in the DB via
// /admin/access-list — never via this array.
//
// Keeping `poum@hjd.builders` here so a fresh local checkout without a
// seeded DB still has a path to authenticate during dev work.
