...
export function EmergencyTips({ category }: EmergencyTipsProps) {
  ...
  return (
    <div className="rounded-2xl border border-orange-700/40 bg-orange-950/30 p-4">
      ...
      <ul className="space-y-2" aria-label="Safety tips while waiting">
        {tips.map((tip, i) => (
          <li key={i} className="flex items-start gap-2 text-sm text-zinc-300">
            <span className="mt-0.5 block h-1.5 w-1.5 shrink-0 rounded-full bg-[#ff4500]" aria-hidden="true" />
            {tip}
          </li>
        ))}
      </ul>
