...
const CONVERSATIONS: Conversation[] = [
  {
    ...
    name: 'Mike Thompson',
    initials: 'MT',
    lastMessage: 'I had to address it immediately to prevent further damage.',
    timestamp: '1h',
    unreadCount: 0,
    avatarColor: '#6366f1',
    isOnline: true,
  ...
  },
  {
    ...
    name: 'Sarah Chen',
    initials: 'SC',
    lastMessage: 'Done! It was a worn washer on the hot side.',
    timestamp: '3h',
    unreadCount: 0,
    avatarColor: '#10b981',
    isOnline: false,
  ...
  },
  {
    ...
    name: 'Dave Martinez',
    initials: 'DM',
    lastMessage: 'Tomorrow morning works. I\'ll be home until noon.',
    timestamp: '4h',
    unreadCount: 0,
    avatarColor: '#64748b',
    isOnline: false,
...
export default function PMMessagesScreen() {
  ...
  useEffect(() => {
    apiFetch<any>('/chat?userId=pm-lisa')
      .then((data) => {
        if (data.conversations?.length > 0) {
          const mapped = data.conversations.map((c: any) => ({
            ...
            isSentByMe: c.lastMessage?.senderId === 'pm-lisa',
            jobTitle: c.jobTitle,
            role: c.otherParticipant?.role === 'pro' ? 'Pro' : 'Tenant',
          }));
  ...
  const handleConversationPress = useCallback((conversation: Conversation) => {
    ...
    // PM uses the pro chat route for now (same component)
    router.push({
      pathname: '/(pm)/chat' as any,
      params: {
  ...
  const renderEmpty = () => (
    <View style={styles.emptyState}>
      ...
      <Text style={styles.emptyDescription}>
        Messages with pros and tenants about work orders will appear here.
      </Text>
    </View>
  );
