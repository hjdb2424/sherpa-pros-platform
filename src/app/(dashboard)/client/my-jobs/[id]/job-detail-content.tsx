...
export function JobDetailContent({ jobId }: JobDetailContentProps) {
  ...
  if (!job) {
    return (
      <div className="px-4 py-16 text-center lg:px-8">
        <h2 className="text-xl font-bold text-zinc-900">Job not found</h2>
        <p className="mt-2 text-sm text-zinc-500">This job may have been removed.</p>
        <Link
          href="/client/my-jobs"
  ...
  return (
    <div className="px-4 py-6 lg:px-8">
      {/* Breadcrumb */}
      <nav className="mb-4 flex items-center gap-2 text-sm text-zinc-400" aria-label="Breadcrumb">
        <Link href="/client/my-jobs" className="hover:text-zinc-600">
          My Jobs
        </Link>
        <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" />
        </svg>
        <span className="truncate text-zinc-600">{job.title}</span>
      </nav>
      ...
      <div className="mb-8">
        <div className="flex items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-xl font-bold text-zinc-900 sm:text-2xl">{job.title}</h1>
            </div>
        ...
        <div className="mt-4 rounded-xl border border-zinc-200 bg-white p-4">
          ...
          <p className="mt-1 text-sm leading-relaxed text-zinc-800">{job.description}</p>
      ...
      {isBidding && (
        <div className="mb-8">
          <div className="mb-4 flex items-center justify-between">
            ...
            {bids.length === 0 && (
              <span className="text-sm text-zinc-400">Waiting for Pros to respond...</span>
            )}
          </div>
          ...
          {bids.length === 0 ? (
            <div className="rounded-xl border border-zinc-200 bg-white p-8 text-center">
              ...
              <p className="text-sm font-medium text-zinc-700">Your job is live</p>
      ...
      {isActive && (
        <div className="space-y-6">
          ...
          {job.assignedPro && (
            <div>
              <h2 className="mb-3 text-lg font-bold text-zinc-900">Assigned Pro</h2>
              <ProCard pro={job.assignedPro} compact />
              <button className="mt-3 flex items-center gap-2 rounded-lg border border-zinc-200 px-4 py-2 text-sm font-medium text-zinc-700 transition-colors hover:bg-zinc-50">
          ...
          {job.milestones && (
            <div>
              <h2 className="mb-3 text-lg font-bold text-zinc-900">Milestone Tracker</h2>
                ...
                {job.milestones.map((ms, idx) => (
                  <div key={ms.id} className="flex items-center gap-4 p-4">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-medium text-zinc-800">{ms.title}</p>
              ...
              <div className="mt-2 flex items-center gap-1 px-1 text-xs text-zinc-400">
          ...
          <div>
            <h2 className="mb-3 text-lg font-bold text-zinc-900">Photo Timeline</h2>
            <div className="rounded-xl border border-zinc-200 bg-white p-8 text-center">
      ...
      {isCompleted && (
        <div className="space-y-6">
          {job.assignedPro && (
            <div>
              <h2 className="mb-3 text-lg font-bold text-zinc-900">Pro</h2>
              <ProCard pro={job.assignedPro} compact />
            </div>
          )}
          ...
          {job.milestones && (
            <div>
              <h2 className="mb-3 text-lg font-bold text-zinc-900">Payment Summary</h2>
              <div className="rounded-xl border border-zinc-200 bg-white divide-y divide-zinc-100">
                {job.milestones.map((ms) => (
                  <div key={ms.id} className="flex items-center justify-between p-4">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm text-zinc-800">{ms.title}</span>
                    </div>
                ...
                <div className="flex items-center justify-between bg-zinc-50 p-4">
                  <span className="text-sm font-bold text-zinc-900">Total Paid</span>
          ...
          {!job.rated && job.assignedPro && (
            <div>
              <h2 className="mb-3 text-lg font-bold text-zinc-900">Leave a Review</h2>
              <RatingForm
