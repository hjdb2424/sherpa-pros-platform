...
**The "licensed + code-aware + true marketplace" quadrant is empty.** Sherpa Pros plants the flag there with built-in Wiseman code intelligence (NEC, IRC, MA Electrical, NH RSA), permit-awareness, rebate-awareness, and a 5–10% take-rate model that is roughly half the effective cost of Angi/Thumbtack lead-gen.
...
| 5 | **Houzz** | 2009; private | **~$4B valuation** (per CB Insights / PitchBook synthesis); $614M total raised from Sequoia / NEA / GGV [^10]; reports of valuation soft markdowns in recent secondary filings [^11]; 2024 industry-wide design-firm revenue decline [^12] | Two products: (a) Houzz consumer marketplace (free directory + lead-gen); (b) **Houzz Pro SaaS** at $59–$999/mo per seat [^13] | Designers, architects, design-build firms; weaker on standalone trades | None at the matching layer | Houzz Pro has 1.03★ BBB average across 500+ complaints in 3 years [^14]; Sitejabber 2.5★ over 159 reviews; contractors report $5K+ spend with zero leads [^14]; lead-blast model (customers contact many pros from one listing); platform is built for inspiration discovery, not execution dispatch | (a) Houzz competes on aspirational visuals and project software; Sherpa Pros competes on **completing the job today**; (b) Wiseman intelligence is execution-grade (code/permit), not portfolio-grade; (c) Sherpa Pros' contractor economics are radically better |
...
| Code intelligence (NEC/IRC/state) | no | no | no | no | no | partial | **yes (Wiseman)** |
...
| Single-pro dispatch (vs. lead-blast) | no | no | yes | yes | no | no | **yes (Dispatch Wiseman)** |
...
4. **Pure SaaS for contractors (Houzz Pro, Buildertrend, JobNimbus).** We do not sell a contractor CRM. Pros use Sherpa Pros to **get jobs**, not to manage their non-Sherpa book. (BldSync — separate product line — is the SaaS for full contractor operations.)
...
1. **Wiseman code intelligence (NEC, IRC, MA Electrical, NH RSA, 192 codes, 480K entries, 284 NH municipalities)** is a multi-year build no national lead-gen incumbent has incentive to replicate — it does not improve their lead-fee economics.
